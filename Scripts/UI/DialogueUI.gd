...
@export_node_path("Control") var backplate_path: NodePath
...
func _autowire_nodes() -> void:
        ...
        if _content == null and _backplate != null:
                _content = _backplate.get_node_or_null("Content") as MarginContainer
        if _anim == null:
...
func _apply_style(style: Dictionary) -> void:
        ...
        if _body != null:
                ...
                if font_path_s != "":
                        var f_res: Resource = load(font_path_s)
                        var fnt: Font = f_res as Font
                        if fnt != null:
                                _body.add_theme_font_override("normal_font", fnt)

        if _content != null:
                if style.has("margin"):
                        var m: int = int(style["margin"])
                ...
                for s in ["left", "top", "right", "bottom"]:
                        var key: String = "margin_" + s
                        if style.has(key):
                                _content.add_theme_constant_override(key, int(style[key]))

        if _backplate != null:
                if style.has("background_texture"):
                        var tex_path_s: String = str(style["background_texture"])
                        var tex: Texture2D = load(tex_path_s) as Texture2D
                        if _backplate.has_method("set_texture"):
                                _backplate.set("texture", tex)
